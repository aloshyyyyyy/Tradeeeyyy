...
# ========= CONFIG =========
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY", "YOUR_GEMINI_KEY")  # paste or set env
...
def save_mistake(text: str, embedding: list[float]):
    with db_conn() as conn:
        c = conn.cursor()
        c.execute(
            "INSERT INTO mistakes (text, embedding, created_at) VALUES (?, ?, ?)",
...
async def handle_mistake(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.lower() == "skip":
        await update.message.reply_text("👍 Skipped. See you next time!")
        return ConversationHandler.END
    try:
        emb = embed_text(text)
        save_mistake(text, emb)
        await update.message.reply_text("✅ Saved! It will be considered next run.")
